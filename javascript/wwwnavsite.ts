async function run() {
    const TYPE_CATEGORY: number = 0,
      TYPE_SUBCATEGORY: number = 1;
  
    let categories: Response = await fetch(
      "resources/navigator_contents.json"
    ...
    let editmode_toggle: HTMLInputElement = <HTMLInputElement>(
      document.getElementById("editmode_toggle")
    );
  
    let current_category: string = Object.keys(categories)[0];
    let current_subcategory: string = Object.keys(
    ...
    function addReferenceCreationButton(): void {
      ...
      reference_add_button.onclick = async function() {
        ...
        categories[current_category][current_subcategory]["references"].push(
          name
        );
        await updateContent();
        updateSite(current_category, current_subcategory);
    ...
    async function init(): Promise<void> {
      ...
      editmode_toggle.addEventListener("click", function() {
        if (editmodeActivated) {
          ...
          category_list.removeChild(
            document.getElementById("category_add_button")
          );
          category_list.removeChild(
            document.getElementById("category_remove_button")
          );
  
          subcategory_list.removeChild(
            document.getElementById("subcategory_add_button")
          );
          subcategory_list.removeChild(
            document.getElementById("subcategory_remove_button")
          );
  
          article_field.removeChild(
            document.getElementById("content_edit_button")
          );
          information_field.removeChild(
            document.getElementById("reference_add_button")
          );
          information_field.removeChild(
            document.getElementById("reference_clear_button")
          );
        } else {
